...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public function delete($id)
    {
        ...
        if ($designation) {
            $designation->delete();        
            $this->dispatch('showMessage', message: 'Designation deleted successfully.');
            $this->dispatch('refreshDesignations');
        } else {
            session()->flash('message', __('Designation not found.'));
        }
    }
...
<div class="flex flex-col space-y-4 p-6">
    ...
    @if ($this->isForm)
        <livewire:designations.form :designationId="$this->designationId"/>
    @endif
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                    @foreach ($this->designations as $designation)
                        <div class="rounded-lg bg-gray-200 p-4 dark:bg-indigo-800 dark:text-indigo-400">
                            <h4><span class="font-semibold">{{ __('ID:') }}</span> {{ $designation->id }}</h4>
                            <p><span class="font-semibold">{{ __('Name:') }}</span> {{ $designation->name }}</p>
                            <p><span class="font-semibold">{{ __('Description:') }}</span> {{ $designation->description }}</p>
                            <p><span class="font-semibold">{{ __('Created At:') }}</span> {{ $designation->created_at->format('Y-m-d H:i:s') }}</p>
                            <p><span class="font-semibold">{{ __('Updated At:') }}</span> {{ $designation->updated_at->format('Y-m-d H:i:s') }}</p>
                            <div class="flex justify-end mt-2">
                                <button
                                    class="bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-600 focus:ring focus:ring-blue-300"
                                    wire:click="createDepartment({{ $designation->id }})"
                                >
                                    {{ __('Edit') }}
                                </button>
                                <button
